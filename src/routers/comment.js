...
router.get("/", (req, res) => {
   ...
   try {
      ...
      const getCommentsQuery = "SELECT idx, comment, user_idx, DATE_FORMAT(CONVERT_TZ(created_at, '+00:00', '+09:00'), '%Y-%m-%d %h:%i %p') AS created_at FROM comment WHERE post_idx = ? ORDER BY idx DESC";
...
router.put("/:commentIdx", (req, res) => {
   ...
   try {
      ...
      const updateCommentQuery = "UPDATE comment SET comment = ?, updated_at = CURRENT_TIMESTAMP WHERE idx = ? AND user_idx = ? AND post_idx = ?";
